...
def initialize_agent(api_key:str)-> tuple[Agent,Agent,Agent,Agent]:
    try:
        model= Gemini(id="gemini-2.0-flash",api_key=api_key)


        therepist_agent = Agent(
            model=model,
            name="Therepist Agent",
            instructions=[
        ...
        return therepist_agent,closure_agent,routine_planner_agent,brutal_honesty_agent
    except Exception as e:
        st.error(f"Error initializing agents: {str(e)}")


## seting page config and Ui elements 


st.set_page_config(
    page_title="💔 Post Breakup Recovery Agents ",
    page_icon="💔",
    layout="wide"
)
...
# Main content
st.title("💔 Post Breakup Recovery Agent")
st.markdown("""
...
if st.button("Get Recovery Plan 💝", type="primary"):
    if not st.session_state.api_key_input:
        ...
        therapist_agent, closure_agent, routine_planner_agent, brutal_honesty_agent = initialize_agent(st.session_state.api_key_input)
...
st.markdown("""
    <div style='text-align: center'>
        <p>Made with ❤️ by the Post Breakup Recovery Agent</p>
        <p>Share your recovery journey with #BreakupRecoveryAgent  #Staystrong  #SleepWell #selfcare # selflove</p>
    </div>
